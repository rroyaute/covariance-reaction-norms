# CRN tuto script
library(cmdstanr); library(shinystan); library(rstan)

# Simulate data
source("CRN functions.R")
#simulate data
stan.dl = sim_CRN_QG(N = 500, Nc = 25, npred = 3, 
                     ntrait = 3, l_es = 0.3, u_es = 0.6)

# Inspect simulated data
str(stan.dl)
plot(stan.dl$z$z1, stan.dl$z$z2)
plot(stan.dl$z$z1, stan.dl$z$z3)
plot(stan.dl$z$z2, stan.dl$z$z3)



# data list (if importing from dataframe)
# stan.dl = list(N = nrow(df), #sample size
#                I = length(unique(df$id)), #number of subjects
#                P = ncol(X), #number of predictors
#                id = df$id, #index subjects per observation
#                X = X, #design matrix of predictors, incl. intercept A = A, #relatedness matrix
#                z1 = df$z1, #response variable 1
#                z2 = df$z2 #response variable 2
# )

# Model matrix (if importing from dataframe)
# X = data.frame(model.matrix(response ~ pred1 + pred2 + pred3 * pred4, data = df))

# Fit simulated data to stan
#compile model
...
est = CRN_mod$sample(
  ...
  max_treedepth = 10,
  refresh = 10)
CRN_fit = read_stan_csv(est$output_files())
saveRDS(CRN_fit, "CRN_fit.RDS")
launch_shinystan(CRN_fit)


# Shinystan report on stored model
y1 = stan.dl$
launch_shinystan(CRN_fit) #GUI for investigating model fit

# Post processing
#name of traits in order of stan.dl$z columns (1, 2, ..., D) #(e.g. "count_egg", "limb.length", "BD1", etc.)
...
#posterior median
aggregate(.~ element, data = v_b, FUN = function(x) round(median(x),2))

#posterior probability in direction of posterior median
aggregate(.~ element, data = v_b, FUN = function(x) sum(sign(median(x))==sign(x))/length(x))

#posterior 90% CIs
aggregate(.~ element, data = v_b, FUN = function(x) round(quantile(x, c(0.05,0.95)),2), simplify = F)
...
#posterior median
aggregate(.~ element, data = cor_b, FUN = function(x) round(median(x),2))

#posterior probability in direction of posterior median
aggregate(.~ element, data = cor_b, FUN = function(x) sum(sign(median(x))==sign(x))/length(x))

#posterior 90% CIs
aggregate(.~ element, data = cor_b, FUN = function(x) round(quantile(x, c(0.05,0.95)),2), simplify = F)
...
ggplot(v_bl2, aes(x = value, color = variable, fill = variable))+
  ...
  facet_wrap(.~element)+
  # labs(x=bquote(bold(paste("log effect size ", sigma[a]ˆ2))))+
  theme(
    legend.position = "top",
